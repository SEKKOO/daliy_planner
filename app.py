...
<head>
  ...
  <style>
    :root {
      ...
      --panel-strong: #ffffff;
      --panel-soft: rgba(244, 249, 255, 0.9);
      --ink: #12304f;
      --muted: #5c7592;
    ...
    * { box-sizing: border-box; }
    ...
    body {
      ...
      background:
        radial-gradient(circle at 10% 8%, var(--accent-glow), transparent 24%),
        radial-gradient(circle at 88% 10%, rgba(86, 168, 255, 0.18), transparent 18%),
        linear-gradient(180deg, var(--bg-soft) 0%, var(--bg) 42%, var(--bg-deep) 100%);
    ...
    .panel::before {
      ...
      inset: 0 0 auto 0;
      height: 1px;
      background: linear-gradient(90deg, rgba(46, 119, 208, 0.3), rgba(46, 119, 208, 0.04));
      pointer-events: none;
    }
    ...
    .weekly-plan {
      ...
      background:
        ...
        linear-gradient(135deg, rgba(46,119,208,0.08), rgba(46,119,208,0));
      border: 1px solid var(--line-soft);
      box-shadow:
    ...
    .primary {
      background: linear-gradient(135deg, var(--accent), #4e9dff);
      color: #fff;
    ...
    .soft {
      background: linear-gradient(180deg, var(--accent-soft), rgba(223, 238, 255, 0.32));
      color: var(--accent-deep);
    ...
    .table-scroll {
      ...
      border-radius: 22px;
      border: 1px solid var(--line-soft);
      background: linear-gradient(180deg, rgba(248,252,255,0.74), rgba(242,248,255,0.58));
    }
    ...
    .item-row {
      display: grid;
      grid-template-columns: 1.12fr 0.96fr 1.62fr 1.26fr 1.08fr 52px;
      gap: 0;
      align-items: stretch;
    }

    .table-header {
      background: linear-gradient(180deg, rgba(225, 238, 255, 0.95), rgba(213, 229, 249, 0.92));
    ...
    .table-header > div {
      min-width: 0;
      padding: 8px 10px;
      border-right: 1px solid rgba(49, 102, 173, 0.12);
      display: flex;
      align-items: center;
      justify-content: center;
      line-height: 1.15;
    ...
    body[data-theme="dark"] textarea {
      background: linear-gradient(180deg, rgba(18, 29, 47, 0.98), rgba(14, 23, 37, 0.94));
      border-color: rgba(128, 170, 231, 0.22);
      color: var(--ink);
    ...
    body[data-theme="dark"] .weekly-pending textarea {
      background: linear-gradient(180deg, rgba(15, 24, 39, 0.98), rgba(11, 18, 30, 0.96));
      border-color: rgba(128, 170, 231, 0.2);
    }

    body[data-theme="dark"] .secondary {
      background: linear-gradient(180deg, rgba(31, 48, 75, 0.96), rgba(22, 35, 56, 0.94));
      border-color: rgba(128, 170, 231, 0.18);
      color: var(--ink);
    }

    body[data-theme="dark"] .soft {
      background: linear-gradient(180deg, rgba(36, 64, 104, 0.96), rgba(25, 46, 76, 0.94));
      border-color: rgba(104, 169, 255, 0.18);
      color: #d8e9ff;
    }
    ...
    body[data-theme="dark"] .table-scroll {
      background: linear-gradient(180deg, rgba(18, 27, 43, 0.92), rgba(15, 23, 38, 0.84));
    }
  ...
  <script>
    ...
    function rowTemplate(item = makeBlankItem(), index = 0) {
      ...
      row.innerHTML = `
        ...
        <div><textarea data-field="completion_status" placeholder="完成情况">${escapeHtml(item.completion_status || "")}</textarea></div>
...
def normalize_item(raw_item: dict) -> dict:
    return {
        ...
        "completion_status": str(raw_item.get("completion_status", "")).strip(),
...
def parse_items(items_json: str, fallback_row: sqlite3.Row | None = None) -> list[dict]:
    ...
    if fallback_row is not None:
        ...
        progress_content = str(fallback_row["progress_content"] or "").strip()
        notes = str(fallback_row["notes"] or "").strip()
        if plan_content or progress_content or notes:
            return [
                {
                    "customer_name": "旧记录",
                    "project_type": "",
                    ...
                    "completion_status": progress_content,
...
def fetch_entry(work_date: str) -> dict | None:
    with get_connection() as connection:
        row = connection.execute(
            """
            SELECT work_date, plan_content, progress_content, notes, created_at, updated_at, items_json
            FROM daily_entries
            WHERE work_date = ?
            """,
            (work_date,),
    ...
    with get_connection() as connection:
        connection.execute(
            ...
            INSERT INTO daily_entries (
                work_date, plan_content, progress_content, notes, created_at, updated_at, items_json
            )
            VALUES (?, '', '', '', ?, ?, ?)
...
5. “明日计划”优先依据下一天已录入的计划；如果下一天没有数据，再根据当天遗留事项和未完成内容做合理整理。
6. “风险和需要协助”需要结合当天工作、遗留事项和风险信息进行分析，归纳可能存在的风险、依赖项、需要协助的事项；没有则写“暂无”。
7. “思考和其他”保持简洁，可补充经验总结、后续关注点；没有则写“暂无”。
8. 日报内容中不要体现项目类型、销售、类型、工时这四类字段，但客户名称、工作内容、完成情况、遗留事项、风险等有效信息需要体现在合适的位置。
9. 不要凭空编造客户、事项、结果；允许做少量明确可解释的整理与归纳。
10. 语言要自然、正式、适合工作日报场景，尽量用条目列出。
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/export-log":
            ...
            try:
                ...
                filename = f"daily-log-{work_date}.md"
                self._send_file(
                    content,
